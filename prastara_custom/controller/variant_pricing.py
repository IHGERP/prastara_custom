...
@frappe.whitelist()
def get_financial_report1(start_date='2025-01-01', end_date='2025-06-30', company=None, cost_center=None, report_type='detailed'):
    """
    Generate a financial report with key metrics, including monthly breakdown and previous year comparison.
    ...
    frappe.log_error(f"Inputs: start_date={start_date}, end_date={end_date}, company={company}, cost_center={cost_center}, report_type={report_type}", 
    ...
    def get_monthly_data(date_start, date_end, company_filter=None, cost_center_filter=None):
        ...
        try:
            ...
            frappe.log_error(f"Monthly query failed: {str(e)}\nQuery: {monthly_query}\nParams: {params}", 
                            "Financial Report Monthly Error")
            return {}, []
    ...
    if not current_monthly_data and not current_raw_data:
        frappe.log_error(f"No data retrieved for current period: {start_date} to {end_date}, company={company}, cost_center={cost_center}", 
                        "Financial Report Data Error")
        return []
    
    if not previous_monthly_data and not previous_raw_data:
        frappe.log_error(f"No data retrieved for previous period: {previous_start_date} to {previous_end_date}, company={company}, cost_center={cost_center}", 
                        "Financial Report Data Error")
        previous_monthly_data = {}
        previous_raw_data = []
