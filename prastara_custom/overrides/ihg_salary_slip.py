...
class IHGSalarySlip(TransactionBase):
	def __init__(self, *args, **kwargs):
		super(IHGSalarySlip, self).__init__(*args, **kwargs)
	...
	def validate(self):
		frappe.errprint("Override triggered")
	...
	def get_emp_and_working_day_details(self):
		...
		if self.employee:
			...
			for x in holidays:
				frappe.errprint("x")
				frappe.errprint(x)
				doc = frappe.db.get_value("Overtime Alloctation",{"ot_date":x,"employee":self.employee},"ot_hours")
				
				frappe.errprint(doc)
				if doc:
					h_ot += doc
					frappe.errprint("doc")
				self.holiday_ot = h_ot


			att_list = frappe.get_all("Attendance", filters = {
						"attendance_date": ["between", [self.start_date, self.end_date]],
						"employee": self.employee,
						"docstatus": 1,
						"status": ["in", ["Present", "Half Day"]]
					}, fields = ["total_missed_hours"])
			
			ot_list = frappe.get_all("Overtime Alloctation", filters = {
			...
			for x in ot_list:
				...
				if x.ot_hours:
					total_ot += x.ot_hours
			frappe.errprint(total_ot)
			if self.holiday_ot:
				self.total_ot = total_ot - self.holiday_ot 
			else:
				self.total_ot = total_ot 
	
			for i in att_list:
				if i.total_missed_hours:
					self.missed_hours += i.total_missed_hours
			...
			# 								order_by='time'
			# 								)
			# 		frappe.errprint(employee_checkin_list)
			...
			# frappe.errprint(self.holiday_ot)
	...
	def get_working_days_details(self, joining_date=None, relieving_date=None, lwp=None, for_preview=0):
		...
		#frappe.errprint("payment1: " + str(payment_days))
...
# there is an issue here with unmarked days, absent and payment days, need to find a solution.
			...
			self.custom_unmarked_days = flt(unmarked_days)
			...
			if payroll_based_on == "Attendance" and consider_unmarked_attendance_as =="Absent":
				...
				#frappe.errprint("rel:" + str(relieving_date))
				if unmarked_days > len(holidays) and not self.relieving_date:
	...
	def get_payment_days(self, joining_date, relieving_date, include_holidays_in_total_working_days):
		...
		frappe.errprint("relieving date:" + str(relieving_date))
		if relieving_date:
	...
	def calculate_lwp_or_ppl_based_on_leave_application(self, holidays, working_days):
		...
		for d in range(working_days):
			...
			leave = frappe.db.sql("""
				...
				AND (t2.is_lwp = 1 or t2.is_ppl = 1 or t2.custom_no_impact_in_payroll = 1)
				AND t1.docstatus = 1
	...
	def calculate_lwp_ppl_and_absent_days_based_on_attendance(self, holidays):
		...
		leave_types = frappe.get_all("Leave Type",
			or_filters=[["is_ppl", "=", 1],["custom_no_impact_in_payroll", "=", 1], ["is_lwp", "=", 1]],
			fields =["name", "is_lwp","custom_no_impact_in_payroll", "is_ppl", "fraction_of_daily_salary_per_leave", "include_holiday"])
		...
		try:
			...
			if condition:
				if not frappe.safe_eval(condition, self.whitelisted_globals, data):
					frappe.errprint("condition comes true")
					frappe.errprint(condition)
					return None
			amount = d.amount
			frappe.errprint("d.amount")
			frappe.errprint(d.amount)
			if d.amount_based_on_formula:
				formula = d.formula.strip().replace("\n", " ") if d.formula else None
				if formula:
					amount = flt(frappe.safe_eval(formula, self.whitelisted_globals, data), d.precision("amount"))
					frappe.errprint("formula called")
