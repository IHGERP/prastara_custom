...
class IHGLoanRepayment(AccountsController):
	def validate(self):
	...
	def book_unaccrued_interest(self):
		...
		if flt(self.total_interest_paid, precision) > flt(self.interest_payable, precision):
			if not self.is_term_loan:
				...
				last_accrual_date = get_last_accrual_date(self.against_loan,posting_date)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		gle_map.append(
			self.get_gl_dict(
				{
					"account": payment_account if payment_account else self.loan_account,
		...
		if gle_map:
			frappe.errprint("gle_map")
			frappe.errprint(self.payment_account)
			frappe.errprint("jkj")
			frappe.errprint(self.payroll_payable_account)
			frappe.errprint(gle_map)
...
def regenerate_repayment_schedule(loan, cancel=0):
	...
	while balance_amount > 0:
		...
		next_payment_date = self.add_single_month(payment_date)
