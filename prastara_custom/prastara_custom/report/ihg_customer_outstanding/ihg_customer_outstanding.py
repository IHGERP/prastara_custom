...
class ReceivablePayableReport(object):
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == "Sales Invoice":
			branch = frappe.db.get_value("Sales Invoice",{"name":row.voucher_no},"branch")
			row.branch = branch
			...
			if self.filters.show_sales_person and row.sales_team:
				row.sales_person_value = ", ".join(row.sales_team)
				del row["sales_team"]

		if self.filters.show_sales_person:
			frappe.errprint("row")
			if row.voucher_type == "Journal Entry":
				sales_person = frappe.db.get_value("Journal Entry",{"name":row.voucher_no},"sales_person")
				row.sales_person_value = sales_person
			if row.voucher_type == "Payment Entry":
				sales_person = frappe.db.get_value("Payment Entry",{"name":row.voucher_no},"sales_person")
				row.sales_person_value = sales_person
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, project_name
	...
	def set_party_details(self, row):
		...
		if self.party_type == "Customer":
			party_details["custom_parent_customer"] = party_details.get("custom_parent_customer") or row.party
	...
	def get_ple_entries(self):
		...
		if self.filters.get("branch"):
			filtered_ple_entries = []
			for entry in self.ple_entries:
				if entry["against_voucher_type"] == "Sales Invoice":
					# Fetch Sales Invoice to check branch
					sales_invoice = frappe.db.get_value("Sales Invoice", {"name":entry["against_voucher_no"]},"branch")
					if sales_invoice != self.filters.get("branch"):
						continue
				filtered_ple_entries.append(entry)

			self.ple_entries = filtered_ple_entries
	...
	def add_customer_filters(
		...
		if self.filters.get("custom_parent_customer"):
			self.qb_selection_filter.append(
				self.ple.party.isin(
					qb.from_(self.customer)
					.select(self.customer.name)
					.where(self.customer.custom_parent_customer == self.filters.get("custom_parent_customer"))
				)
			)
	...
	def get_party_details(self, party):
		if not party in self.party_details:
			if self.party_type == "Customer":
				fields = ["customer_name", "territory","custom_parent_customer", "customer_group", "customer_primary_contact"]
	...
	def get_columns(self):
		...
		if self.filters.party_type == "Customer":
			...
			if self.filters.show_sales_person:
				self.add_column(label=_("Sales Person"), fieldname="sales_person_value", fieldtype="Data")
