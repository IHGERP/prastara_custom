...
#customer-type-filter:focus {
    ...
    setupKeyboardShortcuts() {
        ...
        $(document).on('click', '#gantt-chart-btn', (e) => {
            ...
            const company = encodeURIComponent(
                (this.filters && this.filters.company) || 'PRASTARA DECORATION DESIGN L.L.C'
            );
            window.open(`/app/metro-project-gantt?company=${company}`, '_blank');
        });
    ...
    updateHeaderStats() {
        const isRemainingMetric = this.header_stat_metric === 'remaining_amount';
        const primaryMetricField = isRemainingMetric ? 'remaining_amount' : 'grand_total';
        const secondaryMetricField = isRemainingMetric ? 'grand_total' : 'remaining_amount';
        const primaryMetricLabel = isRemainingMetric ? 'Remaining' : 'Order Value';
        const secondaryMetricLabel = isRemainingMetric ? 'Order Value' : 'Remaining';
        const primaryMetricClass = isRemainingMetric ? 'header-stat-amount-remaining' : 'header-stat-amount-order';
        const secondaryMetricClass = isRemainingMetric ? 'header-stat-amount-order' : 'header-stat-amount-remaining';
        const headerModel = this.getHeaderStatsModel();
        const getRangeMetricAmount = (rangeData, metricField) => metricField === 'remaining_amount' ? rangeData.remainingTotal : rangeData.grandTotal;
        const getOverallMetricAmount = (metricField) => metricField === 'remaining_amount' ? headerModel.ranges.remainingTotal : headerModel.ranges.grandTotal;
        const renderMetricAmounts = (primaryValue, secondaryValue) => `
            <div class="header-stat-amount ${primaryMetricClass}">${primaryMetricLabel}: ${frappe.format(primaryValue, { fieldtype: 'Currency' })}</div>
            <div class="header-stat-amount header-stat-amount-secondary ${secondaryMetricClass}">${secondaryMetricLabel}: ${frappe.format(secondaryValue, { fieldtype: 'Currency' })}</div>
        `;
            ...
            return `
                <div class="header-stat" data-range="${card.key}">
                    ...
                    <div class="header-stat-content">
                        ...
                        ${renderMetricAmounts(
                getRangeMetricAmount(rangeData, primaryMetricField),
                getRangeMetricAmount(rangeData, secondaryMetricField)
            )}
                    </div>
                </div>
            `;
        ...
        const statsHtml = `
            ...
            <div class="header-stat header-stat-summary">
                ...
                <div class="header-stat-content">
                    ...
                    ${renderMetricAmounts(
            getOverallMetricAmount(primaryMetricField),
            getOverallMetricAmount(secondaryMetricField)
        )}
                </div>
            </div>
        `;
    ...
    getHeaderStatsModel() {
        ...
        if (selectedBasis === 'value_range') {
            return {
                ...
                ranges: this.calculateValueRangeMetrics(this.header_stat_metric),
        ...
        return {
            ...
            ranges: this.calculateDateRangeMetrics(selectedBasis),
    ...
    calculateDateRangeMetrics(dateBasis = 'delivery_to_today') {
        const ranges = {
            range0to10: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range10to30: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range30to60: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range60to90: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range90to180: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            rangeAbove180: { count: 0, grandTotal: 0, remainingTotal: 0, orders: [] }
        };

        const addOrderToRange = (bucket, order, grandTotal, remainingTotal) => {
            bucket.count++;
        ...
        this.filtered_orders.forEach(order => {
            ...
            if (daysDifference <= 10) {
                addOrderToRange(ranges.range0to10, order, grandTotal, remainingTotal);
            } else if (daysDifference <= 30) {
                addOrderToRange(ranges.range10to30, order, grandTotal, remainingTotal);
            } else if (daysDifference <= 60) {
                addOrderToRange(ranges.range30to60, order, grandTotal, remainingTotal);
            } else if (daysDifference <= 90) {
                addOrderToRange(ranges.range60to90, order, grandTotal, remainingTotal);
            } else if (daysDifference <= 180) {
                addOrderToRange(ranges.range90to180, order, grandTotal, remainingTotal);
            } else {
                addOrderToRange(ranges.rangeAbove180, order, grandTotal, remainingTotal);
            }
        });

        ranges.grandTotal = this.filtered_orders.reduce((sum, order) => sum + this.toNumber(order.grand_total), 0);
        ranges.remainingTotal = this.filtered_orders.reduce((sum, order) => sum + this.toNumber(order.remaining_amount), 0);
        return ranges;
    }
    ...
    calculateValueRangeMetrics(metricField = 'grand_total') {
        ...
        const ranges = {
            below5k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range5k10k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range10k25k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range25k50k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            range50k100k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] },
            above100k: { count: 0, total: 0, grandTotal: 0, remainingTotal: 0, orders: [] }
        };

        const addOrderToRange = (bucket, order, selectedValue, grandTotal, remainingTotal) => {
            bucket.count++;
        ...
        this.filtered_orders.forEach(order => {
            ...
            if (value < 5000) {
                addOrderToRange(ranges.below5k, order, value, grandTotal, remainingTotal);
            } else if (value >= 5000 && value < 10000) {
                addOrderToRange(ranges.range5k10k, order, value, grandTotal, remainingTotal);
            } else if (value >= 10000 && value < 25000) {
                addOrderToRange(ranges.range10k25k, order, value, grandTotal, remainingTotal);
            } else if (value >= 25000 && value < 50000) {
                addOrderToRange(ranges.range25k50k, order, value, grandTotal, remainingTotal);
            } else if (value >= 50000 && value < 100000) {
                addOrderToRange(ranges.range50k100k, order, value, grandTotal, remainingTotal);
            } else if (value >= 100000) {
                addOrderToRange(ranges.above100k, order, value, grandTotal, remainingTotal);
            }
        });

        ranges.selectedMetric = selectedMetric;
        ranges.selectedMetricTotal = this.filtered_orders.reduce((sum, order) => sum + this.toNumber(order[selectedMetric]), 0);
        ranges.grandTotal = this.filtered_orders.reduce((sum, order) => sum + this.toNumber(order.grand_total), 0);
        ranges.remainingTotal = this.filtered_orders.reduce((sum, order) => sum + this.toNumber(order.remaining_amount), 0);
    ...
    showValueRangeOrders(range) {
        ...
        const title = `${headerModel.titlePrefix}: ${matchedCard.title || matchedCard.label}`;
    ...
    processOrdersData() {
        ...
        this.all_orders.forEach(order => {
            ...
            order.remaining_amount = (!Number.isFinite(apiRemaining) || (billedPercent > 0 && Math.abs(apiRemaining - grandTotal) < 0.01))
                ? computedRemaining
    ...
    fetchDisputeOverviewData() {
        return new Promise((resolve) => {
            frappe.call({
                ...
                args: { company: 'PRASTARA DECORATION DESIGN L.L.C' },
                callback: (r) => {
                    if (r.message && r.message.status === 'success') {
    ...
    showQuickSearchModal() {
        ...
        const getSearchHistory = () => {
            try {
                return JSON.parse(localStorage.getItem('metro_search_history') || '[]');
            } catch (e) {
                return [];
            }
        };
        ...
        const saveToHistory = (query) => {
            ...
            try {
                ...
                localStorage.setItem('metro_search_history', JSON.stringify(history));
            } catch (e) {
